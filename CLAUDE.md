...
- IndexedDB for persistence: database `gtd`, object store `state`, a single versioned document (`version: 3`) written atomically under key `gtd:data`. The legacy `localStorage` key `gtd:data:v1` is migrated on first run and doubles as the fallback when IndexedDB is unavailable or a write fails (init picks whichever copy has the newer `savedAt`). Optionally encrypted at rest behind a biometric/recovery-code device vault (`js/vault.js`); the plaintext path is unchanged when no vault is enrolled.
- PWA: `manifest.webmanifest` + `sw.js` (precache, cache-first). No asset may be loaded from the network at runtime. `index.html` ships a strict Content-Security-Policy (same-origin scripts/styles/workers; `connect-src` allows https + http loopback); keep it in sync when adding an origin the app must reach.
...
5. `js/vault.js` — device vault: **encryption at rest** for the local replica and the sync secrets. A random non-extractable AES-256-GCM data key (DEK) encrypts the whole document (via `store.js`) and the sync config (via `syncer.js`) on disk. The DEK is wrapped per unlock method: a mandatory PBKDF2 recovery code, plus an optional biometric method using the WebAuthn platform authenticator + PRF extension (Touch ID / Face ID / Windows Hello / fingerprint — the browser "keychain"). Device-local (`localStorage: gtd:vault`), never synced or exported. Recovery/at-rest crypto unit-tested via `npm test` (`test/vault.test.js`); the biometric path needs a real browser.
6. `js/syncer.js` — provider-agnostic sync core: any subset of backends can be active at once (redundant cloud peers; the deterministic merge keeps them convergent and a device connected to both bridges devices that only use one), one transport interface (`ensureAuth/list/download/upload`), shared orchestration (encrypted per-device files `gtd-device-<id>.json`, decrypt via `GTD.crypto`, merge via `GTD.sync.merge`, re-upload; backends run sequentially, server first and Google Drive last because its OAuth redirect leaves the page, and one backend failing does not stop the others), a rollback/replay guard (per-file `savedAt` high-water mark, `gtd:sync:hwm`), plus the password-encrypted key file (export/import) for the self-hosted provider. The encryption passphrase is shared by all backends. `sync({interactive})` defaults to interactive (a manual sync may leave the page to re-authorize Google Drive); `app.js` drives it automatically with `interactive: false` (background sync: pushes a debounced pass after every local mutation via `store.subscribe`, and pulls on focus/visibility, a ~90s interval and at boot). A non-interactive sync never triggers the Drive OAuth redirect — the self-hosted server always syncs, but Drive only while its `sessionStorage` token is valid (~1 h), otherwise that backend is skipped (`{skipped: true}`) until the next manual sync. The result carries a top-level `changed` flag so the UI only re-renders when remote data actually merged in. Device-local keys (`localStorage`: `gtd:device-id`, `gtd:sync:config` — includes the passphrase, deliberately device-local; encrypted at rest under the vault DEK in `gtd:sync:config:enc` when a vault is enrolled — and `gtd:sync:last`, per-backend timestamps) are never part of the synced document. `loadConfig()` must run at boot after the vault unlocks. Pure helpers unit-tested via `npm test` (`test/syncer.test.js`).
7. `js/drive.js` — Google Drive transport: OAuth 2.0 Authorization Code flow with PKCE (full-page redirect to `accounts.google.com`, code returned in the query string and exchanged for a token via a background POST bound to a per-attempt `code_verifier`; no external scripts — GIS is a CDN load and stays banned; token + OAuth state + verifier in `sessionStorage`), Drive REST v3 against the user's `appDataFolder`. Each user brings their own OAuth Client ID **and secret** (Google "Web application" clients require it at the token endpoint; device-local, vault-encrypted at rest). Consent requests **offline access** (`access_type=offline&prompt=consent`) so Google returns a **refresh token**; `ensureAuth(config, opts)` then mints fresh access tokens silently (background POST, no redirect), so automatic sync keeps working across sessions. `ensureAuth` returns `{ctx}` synchronously on the cached-token fast path or a Promise otherwise (`{ctx}` after a refresh, `{redirecting}` for an interactive consent, `{unavailable}` for a background sync with no usable token). The refresh token is stored via `syncer.setGdriveRefreshToken` (device-local, in the config, vault-encrypted at rest, never synced; grants offline access only to the user's own `appDataFolder`); a revoked/expired one (`invalid_grant`) is dropped so the next manual sync re-consents. `app.js` must `await GTD.drive.handleRedirect(config)` at boot (async; needs the sync config, so after the vault unlocks). Unit-tested via `npm test` (`test/drive.test.js`).
8. `js/server.js` — self-hosted sync server transport: bearer-key HTTP against the tiny makegtd sync protocol (`GET/PUT {base}/gtd/files[/name]`), configured like a proxy (URL + access key) or via the key file. Reference server + protocol spec live in `server/` (stdlib-only Go, not part of the app). Unit-tested via `npm test` (`test/server.test.js`).
9. `js/model.js` — domain constants (item statuses, labels), factories, pure helpers (overdue/scheduled-today queries, projects without a next action, focus limit).
10. `js/datepicker.js` — shared date/time picker dialog (`GTD.datepicker.open`): single-month grid, optional time behind progressive disclosure, no recurrence. Used by the item editor, the Clarify wizard and the project add-action form.
11. `js/views.js` — render functions for each view (jQuery-built DOM).
12. `js/process.js` — the Clarify wizard (GTD decision tree, one item and one decision at a time).
13. `js/review.js` — the guided weekly review wizard.
14. `js/app.js` — hash router (`#/hoy`, `#/entrada`, …), navigation shell, global quick-capture, service worker registration, and the boot unlock gate (blocks the app behind the vault when encryption at rest is on).

...
- Optional **encryption at rest** (Ajustes → Seguridad del dispositivo): everything on the device — tasks, projects and the sync secrets — is encrypted with a key unlocked by your fingerprint / Face ID (WebAuthn platform authenticator + PRF, the browser keychain) or a recovery code. Off by default; the app runs unencrypted-at-rest until you enable it.
- Optional multi-device sync, end-to-end encrypted on-device (PBKDF2 + AES-256-GCM) before upload, one file per device merged locally (last-writer-wins + tombstones + a rollback guard). Two backends, usable together for redundant backups: your own Google Drive (each user creates their own free OAuth client — id + secret — using the Authorization Code + PKCE flow; Ajustes → Sincronización has the step-by-step guide) and/or a self-hosted sync server (stdlib-only Go reference implementation and protocol spec in `server/`; devices are configured with a URL + access key, or by importing a password-encrypted key file exported from another device). The app stays fully offline-functional with sync off.

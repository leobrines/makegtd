...
- Optional multi-device sync via your own Google Drive: end-to-end encrypted on-device (PBKDF2 + AES-256-GCM) before upload, one file per device merged locally (last-writer-wins + tombstones). Each user creates their own free OAuth client — Ajustes → Sincronización has the step-by-step guide. The app stays fully offline-functional with sync off.

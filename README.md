...
- Optional multi-device sync, end-to-end encrypted on-device (PBKDF2 + AES-256-GCM) before upload, one file per device merged locally (last-writer-wins + tombstones). Two backends: your own Google Drive (each user creates their own free OAuth client — Ajustes → Sincronización has the step-by-step guide) or a self-hosted sync server (stdlib-only Go reference implementation and protocol spec in `server/`; devices are configured with a URL + access key, or by importing a password-encrypted key file exported from another device). The app stays fully offline-functional with sync off.
